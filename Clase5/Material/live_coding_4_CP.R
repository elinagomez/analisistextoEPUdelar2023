...
View(as.data.frame(sesion_corpus_toks_dfm))
...
dfm_intervenciones <- quanteda::dfm(quanteda::tokens(intervenciones$speech,
  ...
  quanteda::dfm_remove(pattern = c(quanteda::stopwords("spanish"),
                      tolower(intervenciones$legislator),"presi*","seño*"),
                       min_nchar=3)%>%
...
View(convert(dfm_intervenciones,to = "data.frame"))
...
quanteda.textplots::textplot_wordcloud(dfm_tfidf(dfm_intervenciones), min.count = 2,max_words = 100,
...
JDM_fcm= dfm_tfidf(kwicJDM) %>%
  fcm(context = "document")

feat <- row.names(head(JDM_fcm, 30)) ##Me quedo con las 30 palabras que más se dicen
